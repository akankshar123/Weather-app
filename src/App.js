...
function App() {
  ...
  const searchWeatherHandler = () => {
    ...
    if (search !== "") {
      axios.get(
        `https://api.openweathermap.org/data/2.5/weather?q=${search}&appid=3ad0a852eff31aed807262bdfe17746e&units=metric`
      )
        .then(
          (response) => {
  ...
  const historySearchHandler = async (data) => {
    ...
    if (data !== "") {
      axios.get(
        `https://api.openweathermap.org/data/2.5/weather?q=${data}&appid=3ad0a852eff31aed807262bdfe17746e&units=metric`
      )
        .then(
          (response) => {
